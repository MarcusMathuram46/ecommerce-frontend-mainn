...
const Checkout = () => {
    ...
    onSubmit: (values) => {
      ...
      setTimeout(() => {
        dispatch(createAnOrder({
          ...
          paymentMethod: paymentMethod === "COD" ? "Payment on delivery" : paymentMethod === "paypal-card" ? "Pay with PayPal" : "",
          shippingInfo: values,
        }))
      }, 500);
    },
  ...
  const onSuccessPaypal = (details, data) => {
    ...
    dispatch(createAnOrder({
      ...
      totalPrice: totalAmount + deliveryPrice,
      orderItems: cartProductState,
      paymentMethod: paymentMethod === "COD" ? "Payment upon delivery" : paymentMethod === "paypal-card" ? "Payment via Paypal" : "",
                            ...
                            <h5 className='total' style={{ color: item?.priceAfterDiscount !== item?.price ? "gray" : "red" }}>
                              {
                                item?.priceAfterDiscount !== item?.price ? <del>
                                  {item?.price * item?.quantity ? (item?.price * item?.quantity).toLocaleString("en-IN", { style: "currency", currency: "INR" }) : "₹"}
                                </del> : item?.price * item?.quantity ? (item?.price * item?.quantity).toLocaleString("en-IN", { style: "currency", currency: "INR" }) : "₹"
                            ...
                            </h5>
                            {
                              item?.priceAfterDiscount !== item?.price && (
                                <div className='d-flex gap-1 justify-content-center'>
                                  ...
                                  {item?.priceAfterDiscount * item?.quantity ? (item?.priceAfterDiscount * item?.quantity).toLocaleString("en-IN", { style: "currency", currency: "INR" }) : "₹"}
                                  </h5>
          ...
          <div className="col-md-4">
            <div className="row">
              <div className="col-sm-6 col-md-12 border-bottom py-4">
                ...
                <div className="form-check">
                  ...
                  />
                  <label htmlFor="COD">Payment on Delivery</label><br />
                  <input
                    type="radio"
                    id="card"
                    name="payment"
                    ...
                    onChange={(e) => { setPaymentMethod(e.target.value) }}
                  />
                  <label htmlFor="card">"Pay with PayPal"</label><br />
                </div>
              </div>
              <div className="col-sm-6 col-md-12 border-bottom py-4">
                ...
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  {totalAmount ? (totalAmount).toLocaleString("en-IN", { style: "currency", currency: "INR" }) : "0 ₹"}
                  </p>
                </div>
                {/* <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  {totalAmount ? (deliveryPrice).toLocaleString("en-IN", { style: "currency", currency: "INR" }) : 0}
                  </p>
                </div>
              </div>
            </div>
            <div>
              <div className="d-flex justify-content-between align-items-center pt-4">
                ...
                <h5 className="total-price">
                {totalAmount ? (totalAmount + deliveryPrice).toLocaleString("en-IN", { style: "currency", currency: "INR" }) : "₹0"}
                </h5>
              </div>
              {
